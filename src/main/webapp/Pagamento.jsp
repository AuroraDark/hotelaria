...
<%}%>
	...
	<form name="frmPagamento" action="pagamento">
		...
		<table>
			...
			<tr>
				<td class="form__group field"><input type="text"
					name="valorTotal" class="form__field" readonly value="<%out.print(request.getAttribute("valorTotal"));%>"><label
					for="valorTotal" class="form__label">Valor Total:</label></td>
			</tr>
			<tr>
				<td class="form__group field"><label for="formaPagamento"
					class="label-select">*Forma de parcelamento</label> <select name="formaPagamento">
						...
						<option value="<%=(request.getAttribute("tipo").equals("1") ? "Débito" : "Crédito")%> - 1 parcela de <%out.print(request.getAttribute("valorTotal"));%>">1 parcela de <%out.print(request.getAttribute("valorTotal"));%></option>
